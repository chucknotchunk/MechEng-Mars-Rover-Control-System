...
// Poll status from the panel deployment system
void requestPanelState(byte slaveAddr) {
  Wire.requestFrom((uint8_t)slaveAddr, (uint8_t)1);  // Request 1 byte from the slave
  if (Wire.available()) {
...
void requestSamplingState(byte slaveAddr) {
  Wire.requestFrom((uint8_t)slaveAddr, (uint8_t)2);  // Request 2 bytes from the slave
  if (Wire.available() == 2) {
...
void requestBatteryState(byte slaveAddr) {
  Wire.requestFrom((uint8_t)slaveAddr, (uint8_t)12);  // Request 12 bytes from the slave
